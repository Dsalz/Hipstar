...
class AddMovie extends Component{
    ...
    newCast = (e) => {
        this.setState({
            cast: [...this.state.cast, {movieName: '', realName: '', pageUrl: ''}]
        })
    }
    ...
    handleCastChange = (e) => {
        ...
        let newStateCast = [...this.state.cast, newCastObj]
        this.setState({
            cast: newStateCast
        })
    }
                ...
                <React.Fragment>
                    ...
                    <section className='movie-details-section-top-info'>
                        ...
                        <div className="movie-details-section-top-info-deets">
                            <input className="bar-input lightskin-input" type="text" name="title" onChange={this.handleChange} value={title}/>
                        </div>
                        <div className="movie-details-section-top-info-btns">
                            <button className="red-cta-btn" onClick={this.showDetailsPage}>Save Movie</button> 
                        </div>
                    </section>
                </section>
                    ...
                    <input className="bar-input lightskin-input add-movie-releaseDate" type="text" name="releaseDate" onChange={this.handleChange} value={releaseDate}/>
                    <h4>Description</h4>
                    <hr className='movie-details-section-red-line'/>
                    ...
                    {cast.map((castMember, i) => (
                        <div className="add-movie-cast-item">
                            <input type="text" name={`movieName-${i}`} value={castMember.movieName} placeholder="Movie Name" onChange={this.handleCastChange}/> 
                            <input type="text" name={`realName-${i}`} value={castMember.realName} placeholder="Real Name" onChange={this.handleCastChange}/> <br />
                            <input type="text" name={`pageUrl-${i}`} value={castMember.pageUrl} placeholder="Page Url" onChange={this.handleCastChange}/> 
                        </div>
                    ))}
